...
export default function StoreLayout() {
  ...
  const lidarComSalvarZona = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formName.trim()) {
  ...
  const lidarComExcluirZona = async (id: number) => {
    if (!window.confirm('Tem certeza que deseja excluir esta seção do layout?')) return;
    try {
  ...
  const lidarComAssociarItem = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const obterEstiloCalor = (sales: number) => {
  ...
  // Renderiza ícone com base no tipo da seção
  const renderizarIconeTipo = (type: string) => {
    switch (type) {
  ...
  const obterNomeTipo = (type: string) => {
    switch (type) {
      ...
      <div className="grid-2col-1-15">
        ...
        <div className="flex-col gap-3">
          <div className="glass-card" style={{ padding: '20px' }}>
            ...
            >
              ...
              >
                ...
                {zones.map((zone) => {
                  ...
                  // Define a altura (Z-extrusion) baseada no tipo da gôndola/seção
                  const H = zone.zone_type === 'fridge' ? 65 
                          : zone.zone_type === 'shelf' ? 45 
                          : zone.zone_type === 'checkout' ? 25 
                          : zone.zone_type === 'bakery' ? 35 
                          : zone.zone_type === 'butcher' ? 35 
                          : zone.zone_type === 'hortifruti' ? 20 
                          : 30;

                  if (is3dMode) {
                    // Renderização tridimensional (extrusão CSS 3D)
                    return (
                      <div
                        key={zone.id}
                        style={{
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            width: '100%',
                            height: '100%',
                            backgroundColor: heatmapMode ? undefined : (zone.color || '#4f46e5'),
                            backgroundImage: heatmapMode ? undefined : `linear-gradient(135deg, ${zone.color}ee 0%, ${zone.color}bb 100%)`,
                            ...obterEstiloCalor(zone.sales_30_days),
                    ...
                    // Renderização bidimensional plana tradicional
                    const estiloBase = {
            ...
            {is3dMode && (
              <div 
                className="flex flex-wrap items-center gap-3 p-3 rounded-lg border mt-3 text-xs"
        ...
        <div className="flex-col gap-3">
          {/* 1. Modo Edição / Criação de Seção */}
          {isEditing ? (
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              <form onSubmit={lidarComSalvarZona} className="flex-col gap-4">
                ...
                <div style={{ border: '1px dashed var(--border)', borderRadius: '8px', padding: '16px', background: 'rgba(0,0,0,0.1)' }} className="flex-col gap-3">
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      <label className="text-xs">Coluna (X)</label>
                      <input
                        type="number"
                        min="0"
                        max="15"
                    ...
                    <div className="form-group">
                      <label className="text-xs">Linha (Y)</label>
                      <input
                        type="number"
                        min="0"
                        max="11"
            ...
            /* 2. Visualização de Seção Selecionada */
            <div className="glass-card" style={{ padding: '24px' }}>
              <div className="flex-between mb-3" style={{ borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
                <div>
                  <span className="badge info text-uppercase mb-1" style={{ fontSize: '0.65rem' }}>
              ...
              <div className="flex-col gap-2 mb-4">
                <span className="font-bold text-xs text-muted block mb-1">Itens e Categorias Posicionados</span>
                ...
                <form onSubmit={lidarComAssociarItem} style={{ border: '1px dashed var(--border)', borderRadius: '8px', padding: '16px', background: 'rgba(0, 0, 0, 0.15)' }} className="flex-col gap-3">
                  ...
                  <div className="form-group">
                    <label className="text-xs">Por Categoria Inteira</label>
                    <select
                      value={associarCatId}
                      onChange={(e) => {
                  ...
                  <div className="form-group">
                    <label className="text-xs">Por Produto Específico</label>
