...
export default function POS({ currentUser, activeSession, onLogout, onCloseCashSession }: POSProps) {
  ...
  const [nfcReceipt, setNfcReceipt] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    setCart((prevCart) => {
      ...
      const updatedCart = prevCart.map((item) => {
        ...
        if (latestProd) {
          const latestPrice = latestProd.promotional_price !== undefined ? latestProd.promotional_price : latestProd.price_sell;
          if (item.price_unit !== latestPrice || item.product.active_promotion?.name !== latestProd.active_promotion?.name) {
            ...
            return {
              ...
              price_total: parseFloat((item.quantity * latestPrice).toFixed(2))
            };
          }
        }
        return item;
  ...
  const totalPaid = isSplitMode
    ? Object.entries(splitAmounts).reduce((sum, [_, val]) => sum + (parseFloat(val) || 0), 0)
    : (activeMethod?.type === 'dinheiro'
        ? (parseFloat(singleCashPaid) || finalTotal)
        : finalTotal);
  ...
  const changeGiven = isSplitMode
    ? (totalPaid > finalTotal ? Math.max(0, totalPaid - finalTotal) : 0)
    : (activeMethod?.type === 'dinheiro' && parseFloat(singleCashPaid) > finalTotal
        ? parseFloat(singleCashPaid) - finalTotal
        : 0);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Block common reload and back/forward navigation keys
      if (
        e.key === 'F5' || 
      ...
      if (e.key === 'F1') {
        ...
        if (cart.length > 0) {
          ...
          // Auto focus amount input if cash is selected
          setTimeout(() => {
      ...
      if (isPaymentModalOpen) {
        if (e.key === 'Enter') {
          ...
          if (isSplitMode) {
            ...
            if (activeId) {
              ...
              if (idx !== -1) {
                ...
                const nextIdx = (idx + dir + enabledMethods.length) % enabledMethods.length;
                const nextMethodId = enabledMethods[nextIdx]?.id;
                if (nextMethodId) {
                  splitInputRefs.current[nextMethodId]?.focus();
                  splitInputRefs.current[nextMethodId]?.select();
                }
              }
            }
          } else {
            ...
            if (idx !== -1) {
              const dir = e.key === 'ArrowDown' ? 1 : -1;
              const nextIdx = (idx + dir + enabledMethods.length) % enabledMethods.length;
              const nextMethod = enabledMethods[nextIdx];
              if (nextMethod) {
        ...
        } else if ((e.key === '1' || e.key === '2' || e.key === '3' || e.key === '4') && document.activeElement?.tagName !== 'INPUT') {
          e.preventDefault();
          ...
          if (isSplitMode) {
            if (num >= 0 && num < enabledMethods.length) {
  ...
  const addToCart = (product: Product) => {
    const currentPrice = product.promotional_price !== undefined ? product.promotional_price : product.price_sell;
    ...
    setCart((prevCart) => {
      ...
      if (existingIdx > -1) {
        ...
        updatedCart[existingIdx] = {
          ...
          price_total: parseFloat((newQty * item.price_unit).toFixed(2))
        };
        return updatedCart;
      } else {
        // Add new item
        return [
          ...prevCart,
          {
            ...
            price_total: parseFloat((qtyToAdd * currentPrice).toFixed(2))
          }
        ];
      }
    });
  ...
  const updateQuantity = (productId: number, val: number) => {
    ...
    setCart((prevCart) =>
      prevCart.map((item) => {
        ...
        return {
          ...item,
          quantity: parseFloat(val.toFixed(3)),
          price_total: parseFloat((val * item.price_unit).toFixed(2))
        };
      })
    );
  };
    ...
    try {
      ...
      if (res.success) {
        if (cancelType === 'sale') {
          ...
          // Remove single item
          const idx = parseInt(cancelItemIndex) - 1;
  ...
  async function handleFinishSale() {
    ...
    // Build the mapping of payment methods used
    const salePayments: Record<string, number> = {};
    if (isSplitMode) {
      paymentMethods.forEach(m => {
        const amt = parseFloat(splitAmounts[m.id]) || 0;
        if (amt > 0) {
    ...
    // Validation: check if totalPaid is enough
    ...
    // Identify if fiado type is used and check constraints
    ...
    // Determine main payment method type
    ...
    const salePayload: Omit<Sale, 'id' | 'created_at'> = {
      ...
      amount_paid: parseFloat(totalPaidSum.toFixed(2)),
      change_given: parseFloat(localChange.toFixed(2)),
      fee_amount: parseFloat(totalFee.toFixed(2)),
      items: cart.map(item => ({
    ...
    try {
      ...
      if (nfcResultObj) {
        ...
        const copies = parseInt(localStorage.getItem('superpos_printer_copies') || '1');
        ...
        // Reset cart and states immediately (conclude sale automatically)
        setCart([]);
        setLatestItem(null);
  ...
  const handleCloseShift = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cashReported = parseFloat(finalCashReported) || 0.0;
      const cardReported = parseFloat(finalCardReported) || 0.0;
  ...
  const filteredProductsList = searchProductQuery
    ? products.filter(p => 
        ...
        (p.code && p.code.toLowerCase().includes(searchProductQuery.toLowerCase())) ||
        (p.barcodes && p.barcodes.some(b => b.includes(searchProductQuery)))
      ).slice(0, 5)
    : [];
    ...
    <div className="pos-fullscreen-container animate-fade-in">
      ...
      <header className="pos-fullscreen-header">
        <div className="pos-fullscreen-header-title">
          ...
          <span>SuperPOS <strong style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>CHECKOUT</strong></span>
        </div>
            ...
            <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '12px' }}>
              ...
              <div className="flex-between gap-1.5">
                ...
                <input 
                  ...
                  onChange={(e) => setDiscountPercent(Math.min(100, Math.max(0, parseInt(e.target.value) || 0)).toString())}
      ...
      {isPaymentModalOpen && (() => {
        ...
        if (pixMethod) {
          if (isSplitMode) {
            pixAmount = parseFloat(splitAmounts[pixMethod.id]) || 0;
                  ...
                  <>
                    {activeMethod?.type === 'dinheiro' && (
                      <div className="form-group">
                        <label className="block text-xs text-muted mb-2 font-bold uppercase">Valor Recebido (R$)</label>
                        <input 
                          ref={payAmountRef}
                          type="number"
                          step="0.01"
                ...
                <div className="p-3 rounded bg-white/5 border border-gray-850" style={{ display: 'flex', flexDirection: 'column', gap: '8px', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '6px' }}>
                  ...
                  {emitNfc && (
                    <div className="animate-fade-in" style={{ marginTop: '4px' }}>
                      <label className="block text-[10px] text-muted mb-1 font-bold uppercase" style={{ display: 'block', fontSize: '9px', opacity: 0.7 }}>CPF na Nota (Opcional)</label>
                      <input 
                        id="cpf-input-field"
                        type="text"
                ...
                {isSplitMode ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <label className="block text-xs text-muted mb-0 font-bold uppercase">Múltiplos Pagamentos</label>
                      <button 
                  ...
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <label className="block text-xs text-muted mb-0 font-bold uppercase">Forma de Pagamento</label>
                      <button 
                        onClick={toggleSplitMode}
        ...
        <div className="pos-payment-modal-overlay">
          <div className="manager-auth-modal animate-fade-in">
            ...
            <form onSubmit={handleManagerAuth} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {cancelType === 'item' && (
                <div className="form-group">
                  <label className="block text-xs text-muted mb-2 font-bold uppercase">Posição do Item (1, 2...)</label>
                  <input
                    ref={cancelIndexRef}
                    type="number"
                    min="1"
              ...
              <div className="form-group">
                <label className="block text-xs text-muted mb-2 font-bold uppercase">Senha do Gerente</label>
                <input
        ...
        <div className="pos-payment-modal-overlay">
          <div className="close-cash-modal animate-fade-in" style={{ maxWidth: '480px' }}>
            ...
            <form onSubmit={handleCloseShift} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div className="form-group">
                <label className="block text-xs text-muted mb-2 font-bold uppercase">Dinheiro Físico Contado na Gaveta (R$)</label>
                <input
              ...
              <div className="form-group">
                <label className="block text-xs text-muted mb-2 font-bold uppercase">Valor Total em Cartões Passado nas Máquinas (R$)</label>
                <input
                  type="number"
                  step="0.01"
                  className="input-field font-bold text-center text-lg text-monospace"
              ...
              <div className="p-2 rounded text-center font-bold text-sm" style={{ background: 'rgba(255,255,255,0.05)', border: '1px dashed var(--border)' }}>
                Soma Total Informada: {formatCurrency((parseFloat(finalCashReported) || 0) + (parseFloat(finalCardReported) || 0))}
              </div>

              <div className="form-group">
                <label className="block text-xs text-danger mb-2 font-bold uppercase">Senha de Autorização do Gerente</label>
                <input
                  type="password"
        ...
        <div className="pos-payment-modal-overlay" style={{ zIndex: 999 }}>
          <div className="close-cash-modal animate-fade-in" style={{ maxWidth: '360px', padding: '24px' }}>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="form-group">
                <label className="block text-xs text-muted mb-2 font-bold uppercase text-center">Digite a quantidade multiplicadora:</label>
                <input
        ...
        <div className="pos-payment-modal-overlay" style={{ zIndex: 999 }}>
          <div className="close-cash-modal animate-fade-in" style={{ maxWidth: '360px', padding: '24px' }}>
            ...
            <form onSubmit={handleManualCodeSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="form-group">
                <label className="block text-xs text-muted mb-2 font-bold uppercase text-center">Digite o código ou código de barras:</label>
                <input
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '8px' }}>
                  ...
                  {nfcReceipt.items.map((item: any, idx: number) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: printerWidthVal === '58mm' ? '9px' : '11px' }}>
